...
// 实现查找对应虚拟地址对应的（二级）页表项，根据if_create判断在原先没有分配页的是否分配一页
...
// 用于查找虚拟地址对应的页控制块及页表项，存储到pte_pointer中
...
void page_remove(Pde *pgdir, u_int asid, u_long va) {
	...
	struct Page *page_pointer = page_lookup(pgdir, va, &pte);
